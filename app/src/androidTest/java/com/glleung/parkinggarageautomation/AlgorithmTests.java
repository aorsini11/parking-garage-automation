...
@LargeTest
public class AlgorithmTests extends ActivityInstrumentationTestCase2<MainLogin>{
   ...
   public void setUp() throws Exception {
    ...
    public void TestCreateReservation() {
        Firebase.setAndroidContext(mMainActivity);
        handled = false;
        Firebase reservationRef = new Firebase("https://incandescent-fire-3535.firebaseio.com/reservations/");
        Map<String,Object> map = new HashMap<String, Object>();
        map.put("userid","ca55364c-a415-4512-a950-85108631862a");
        map.put("carid",1);
        map.put("startdate","2-27-3-2017");
        map.put("enddate","10-28-3-2017");
        map.put("price",50.00);

        Firebase reservation = reservationRef.push();
        reservation.setValue(map);
        reservation.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
                String id = (String) dataSnapshot.child("carid").getValue();
                String startdate = (String) dataSnapshot.child("startdate").getValue();
                String enddate = (String) dataSnapshot.child("enddate").getValue();

                Assert.assertEquals(id,"ca55364c-a415-4512-a950-85108631862a");
                Assert.assertEquals(startdate,"2-27-3-2017");
                Assert.assertEquals(enddate,"10-28-3-2017");
            }
